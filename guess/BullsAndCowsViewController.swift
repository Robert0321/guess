...
class BullsAndCowsViewController: UIViewController {
    ...
    func checkAnswer() {
        ...
        if numberOfGuess%2 == 0 {
            moonImageViews[moonIndex].image = UIImage(systemName: "")
            moonIndex -= 1
        } else {
            moonImageViews[moonIndex].image = UIImage(systemName: "moon")
        }
    ...
    func startNewGame() {
        // 重設月亮及猜的次數
        moonIndex = 5
        for moon in moonImageViews {
            moon.image = UIImage(systemName: "moon.fill")
        }
        numberOfGuess = 0
        // 重設鍵盤及遊戲畫面
        resetNumberKeyboard()
